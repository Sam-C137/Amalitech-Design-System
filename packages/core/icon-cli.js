...
const BUILD = require('./src/scripts/files.js');
const { red, yellow, bgWhite, blue, green, bold } = require('kleur');
const { kebabCase, fileName } = require('./src/scripts/helpers.js');
    ...
    settings.filelist.map(async srcFileName => {
      try {
        await getFileData(srcFileName).then(fileData => {
          const optimizedSVG = optimize(fileData, {
            path: srcFileName,
            multipass: true,
            removeAttrs: {
              attrs: ['xmlns'],
...
const buildFile = async (file, ext, data) => {
  await fsp.writeFile(path.join(settings.dest, fileName(file.name), kebabCase(fileName(file.name)) + ext), data);
};
